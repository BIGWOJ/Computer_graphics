    ...
    for(int j = 0; j < img.rows; j++) {
        for(int i = 0; i < img.cols; i++) {
            ...
            if(is_gamma_correction) {
                //zmiana wartości korekcji gamma
                float gamma = 2.1489f;
...
void CImage::plotCalibChart() {
    ...
    for (int i = 0; i < 5; i++) {
        float kolor = i * 0.25f;
        cv::Point pp1 = {i * rozmiar + krok, 300};
        cv::Point pp2 = {(i + 1) * rozmiar + krok, 395};
        cv::rectangle(getImage(), pp1, pp2, cv::Scalar(kolor, kolor, kolor), -1);
        krok += 5;
    }
}
